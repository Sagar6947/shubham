...
if (isset($_POST["sell_property"])) {
    ...
    $rt = "UPDATE `tbl_property_sell` SET `update_date`= CURRENT_TIMESTAMP, `ba_id`='$ba_id' , `user_id`='$user_id' , `property_name`='$property_name',`registration_no`='$registration_no',  `property_category`='$property_category',`property_type`='$property_type', `flat_bhk`='$flat_bhk',`property_price`='$property_price ',`address`='$address', `state`='$state',`city`='$city',`zipcode`='$zipcode',`area`='$area',`map`='$map', `plot_area`='$plot_area',`builtup_area`='$builtup_area',`built_year`='$built_year', `furnished`='$furnished',`transaction_type`='$transaction_type',`landmarks`='$landmarks',`ownership`='$ownership', `amenities`='$amenities', `liabilities`='$liabilities', `terms`='$terms'  ,`bedrooms`='$bedrooms',`bathrooms`='$bathrooms',`total_floor`='$total_floor',`facing`='$facing' , `youtube`='$youtube',`price_cr`='$price_cr', `price_val`='$price_val'  WHERE `sell_id` = '$pro_id'";
